...
import { X, ChevronUp, ChevronDown, Upload, Download, XCircle, Check, Loader2, Zap } from "lucide-react";
import type { Transfer } from "@/lib/agent";
...
export function TransferPanel({ transfers }: Props) {
  ...
  const done = transfers.filter((t) => t.status !== "running");
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-sm">
          <span className="truncate font-medium">
            {t.currentFile ? t.currentFile.split("/").pop() : t.direction === "upload"
        ...
        <div className="mt-0.5 truncate font-mono text-[11px] text-muted-foreground">
          {t.direction === "upload" ? "↑" : "↓"} {t.direction === "upload" ? t.localPath : t.remotePath}
          {" → "}
        ...
        {t.status === "running" && (
          <div className="mt-1.5">
            ...
            <div className="mt-1 flex justify-between font-mono text-[10px] text-muted-foreground">
              <span>{t.progress || 0}% · {formatBytes(t.transferred || 0)}</span>
              <span>{t.speed || "—"}</span>
            </div>
          </div>
        )}
        {t.status === "failed" && t.error && (
          <div className="mt-1 truncate font-mono text-[10.5px] text-destructive/80">
            {t.error}
          </div>
        )}
      </div>
